...
trait zModSqlBuilderTra
{
	...
	public static function setDb($dbName, $reference){
		...
		self::$db = $dbName;
		self::$realDb = self::parseRuleMap(PARSE_TYPE_DB, self::$dbRules[$dbName][0], $reference, $dbName);
		self::$dbModelType = self::$dbRules[$dbName][1];
		return self::$_instance;
	}
	...
	public static function setTable($tableName, $reference){
		if(!isset(self::$tableRules[self::$db])){
			self::$tableRules[self::$db] = zCoreConfigCls::loadConfig('tableRules_' . self::$db);
		}
	...
	private static function parseRuleMap($type, $ruleCode, $reference = '', $orgName = ''){
		...
		switch($ruleCode){
			...
			case MAP_RULE_MOBILE_FOURTH://手机号码第四位
				...
				break;
			case MAP_RULE_YEAR:
				$value = zPubBasicLib::date('Y', $reference);
				break;
			case MAP_RULE_MONTH:
				$value = zPubBasicLib::date('m', $reference);
				break;
		}
		if($type == PARSE_TYPE_PARTITION){
			return is_numeric($value) ? (int)$value : $value;
		}
		return ($type == PARSE_TYPE_TABLE ? self::$prefix : '') . $orgName . '_' . str_pad($value, 5, 0, STR_PAD_RIGHT);
	}
